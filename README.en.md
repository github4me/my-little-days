...
- **Calendar:** Records opens in Calendar by default, showing feeds, nappies, and sleep together in a day or week timeline. Jump to a date, filter by type, and tap an entry to view, edit, or delete with confirmation. Overlapping records remain separate; midnight-spanning sessions appear on each relevant day. The existing Bar chart remains available. More saves the default view immediately; switching inside Records is temporary.
