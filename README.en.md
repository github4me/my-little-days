...
- **Calendar:** Records defaults to the unchanged Bar chart. A right-aligned, two-icon toggle switches between Calendar and Bar chart. In Calendar, Day, Week, and Today sit on the left of the same toolbar. See feeds, nappies, and sleep together, jump to a date, filter by type, and tap an entry to view, edit, or delete with confirmation. Overlapping records remain separate; midnight-spanning sessions appear on each relevant day. More saves the default view immediately; existing saved preferences are retained and switching inside Records is temporary.
- **Version footer:** each page displays the app version and build number to help identify the installed release.
...
- **Privacy and support:** in-app privacy information, support contact, and credits thanking Trista (FPH), Mia, Violet, Bill, and other supporters. Activity guidance is for parents, not a screen-based course for babies or a developmental assessment.
