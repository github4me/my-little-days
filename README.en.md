...
Security candidate 0.2.1 requires a new native build: shared photos are sanitized, failed family-reminder cleanup is retried, and iOS SQLite data is excluded from OS backups. Personal records can be exported manually; active family members can download confirmed server records. Remote OTA execution is disabled in this candidate. See the [security remediation checklist](docs/SECURITY-REMEDIATION-2026-09-17.md) for API limits, deployment steps and the still-open recovery acceptance gate.
...
- **Backups:** personal mode supports validated JSON export/import; imports replace personal records after confirmation, not merging them. Every active family member can download the latest confirmed server snapshot for backup or personal analysis. Family files are download-only: importing/restoring them is not supported.
...
- Leaving or removal revokes server access immediately; contributions remain with the family. Devices purge family cache and unsent work when revocation is detected. Admins can remove members without advance notice. Active members can explicitly download confirmed server records, but cannot import files into the shared family or copy old family content into a new family through the app. Saved or shared exports cannot be recalled after access is revoked.
...
Offline records remain in local SQLite without an account. Signing in alone does not upload them. Explicit family creation uploads the reviewed history, current baby avatar, reminder rules/settings and play selections/check-ins. Joining downloads and durably saves the family data before replacing the original local content; it never uploads or merges the joiner’s personal data. Shared cache and saved queues are isolated from the personal database and are not the source for family exports: each download requires a fresh authorized server snapshot. Device preferences, notification permission and notification delivery remain local; each phone must opt into family reminders.

This security candidate does not accept remote code updates. On iOS its SQLite directory, including personal records and family cache/queues, is excluded from OS backups. This does not exclude files you save outside the app from your chosen storage provider's backups. Older system backups and copies cannot be recalled by an app update.

In **More → Backup and restore**, personal mode exports unencrypted JSON containing the local baby profile, tracking entries and daily-care records, but not the avatar, preferences, reminders, play data or family credentials. Personal imports preview the file before confirmed replacement. Export personal records before uninstalling or changing phones.

In family mode, every active owner or caregiver can select **Download and export family backup** while online. On iPhone, choose **Save to Files** in the share sheet and verify the file was saved. This separate JSON format contains the latest confirmed server profile, records, photo, shared reminders and play data, with family/history identifiers, record versions and authorship. It is not anonymized or encrypted. Member rosters, invitation emails, tokens, pending local edits and unresolved conflict proposals are excluded. Downloads do not change shared records. Family-file import/restore is not supported, including into personal mode.

Keep exports private and share them only deliberately; choosing a cloud destination uses that service. Signing out, leaving/removal, family/account deletion or uninstalling cannot recall files already saved or shared. Members must manage those copies themselves. Uninstalling does not delete the server account or family records; use the in-app account/family controls for those actions.
