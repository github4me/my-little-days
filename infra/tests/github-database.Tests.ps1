...
function Reset-Test {
    $global:dbTest = @{ Rules=@(); Calls=[Collections.Generic.List[string]]::new(); FailApply=$false; FailCreate=$false; FailCleanup=$false; FailCheck=$false; WrongIdentity=$false; Ip='20.30.40.50' }
...
try {
    ...
    Run-Case { $global:dbTest.Rules=@(@{name='app-outbound';startIpAddress='20.21.22.23';endIpAddress='20.21.22.23'}) } $false {
    ...
    Write-Output 'PASS: 11 database firewall cases plus recovery; no Azure requests were made.'
} finally {
