...
test('only manual reviewed release can reach migration then API', () => {
  ...
  assert.match(JSON.stringify(release.jobs.validate.steps), /current.data.object.sha !== sha/);
});
...
test('CI runs both real-SQL test projects and publishes the standalone migrator', () => {
  ...
  assert.ok(steps.some(x => x.run?.includes('dotnet test server/LittleDays.slnx')));
  assert.ok(steps.some(x => x.run?.includes('github-database.Tests.ps1')));
  assert.ok(steps.some(x => x.run?.includes('dotnet publish server/LittleDays.DatabaseMigrator/')));
  assert.ok(steps.some(x => x.with?.name === 'family-database-${{ github.run_id }}'));
});
test('API has no migration execution or embedded schema files', () => {
  assert.ok(!read('server/LittleDays.FamilyApi/Program.cs').includes('MigrateAsync'));
  assert.match(read('server/LittleDays.FamilyApi/LittleDays.FamilyApi.csproj'), /Compile Remove="Migrations\/\*\*\/\*.cs"/);
  assert.ok(!read('server/LittleDays.FamilyApi/LittleDays.FamilyApi.csproj').includes('dbup'));
});
