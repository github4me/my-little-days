...
For this initial release, configure a dedicated confidential Graph application in the customer tenant using direct server-side App Service settings `AccountDeletion__GraphClientId` and `AccountDeletion__GraphClientSecret`. Key Vault is optional. Keep the secret blank in `infra/api-appsettings.example.json` and any local template copies; enter the actual value only in restricted server configuration. Exclude credentials from mobile/EAS configuration, Git, build/deployment artifacts and logs. Restrict App Service configuration and deployment access, record the secret's expiry, and rotate it manually before expiry; verify deletion after rotation.

Without valid Graph credentials, SQL cleanup can run but identity deletion remains pending. Configure and verify identity deletion before admitting real users. Status is checked with a device-only secret receipt; no login must survive for completion polling. Follow the [directory cleanup and release gates](../docs/AZURE-FAMILY-PILOT-SETUP.md#6a-enable-account-identity-deletion-server-only): live Graph permissions, static admission configuration cleanup, backup retention/restore replay and native validation are mandatory before real-user release.
