...
CI run 67 (22 September 2026, commit `f32c1e1`) exposed an overly broad migration-test assertion: the shared-extras test expected exactly two enabled/trusted table constraints, but migration `0009` correctly added a third for conflict-replacement audit data. All 266 API tests and 79 of 80 migrator tests passed. Scope feature-specific assertions to the intended constraint names while retaining enabled/trusted checks and behavioral rejection tests; do not remove the new constraint or edit an immutable migration to satisfy an old count. Local TypeScript verification does not run the .NET/real-SQL suite; require separate SQL-backed CI evidence.
