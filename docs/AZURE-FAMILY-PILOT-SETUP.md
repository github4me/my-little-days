...
2. Create a short-lived client secret for this confidential app. Put it in Azure Key Vault, never Git, EAS/mobile environment variables, command history or screenshots. Grant the App Service identity only the required secret-read access in the hosting tenant. Set `AccountDeletion__GraphClientId` to this app's GUID and `AccountDeletion__GraphClientSecret` using an App Service Key Vault reference. Verify that Azure resolves the reference; unresolved references fail startup. Set rotation/expiry alerts and test rotation. [App Service Key Vault references](https://learn.microsoft.com/en-us/azure/app-service/app-service-key-vault-references).
