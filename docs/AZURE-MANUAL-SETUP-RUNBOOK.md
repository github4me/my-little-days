...
| Customer admission compatibility | Added strict support for the observed `creationType=null`, `federated`/`mail` OTP account format | Regression tests pass; live read-only Graph lookup returns exactly the same enabled account. Deploy the correction and verify native sign-in; do not recreate the account |
