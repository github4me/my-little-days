...
Updated 16 September 2026. This is the current activation guide for the full family service, replacing the former synthetic-data setup guide. Completed database/API and preview publication are recorded in the [release evidence](AZURE-MANUAL-SETUP-RUNBOOK.md#shared-extras-release-evidence-16-september-2026); this setup guide does not establish successful native device acceptance.

For portal-by-portal instructions, recorded resource/client IDs, setup status and common-error recovery, use the [manual Azure and GitHub setup runbook](AZURE-MANUAL-SETUP-RUNBOOK.md). It distinguishes confirmed setup from remaining operator verification and does not change or restart running releases.

The native app and `/v2` API share the baby profile, feeding, nappies, sleep, growth, milestones and daily care, including temperature, plus the current baby avatar, reminder rules/settings and play selections/check-ins. The first owner reviews and uploads those data; after verification and durable storage of the family snapshot on the device, successful activation clears the creator's original personal data and recovery copies without retaining a recoverable personal copy. Invitees explicitly accept, download and durably save that family before their original local data and recovery copies are cleared and replaced; personal data is never uploaded or merged on joining. Later invitations use the existing family. Invitations appear in the recipient's in-app inbox: **no email or invitation notification is sent**, and no delivery service is required. Active non-admin members and distinct unexpired pending invitations share five places. Device preferences and notification permission remain local, and each phone must separately enable notification delivery. The device photo library is not uploaded. See [shared data and activation](FAMILY-EXTRAS.md).

Deploy the database migration and compatible API before the mobile update. `/v1` remains for identity and membership lifecycle routes; the old bottle-feed-only contract cannot activate a full family. Authenticated `/v2/capabilities` must advertise schema `2`, all six history record kinds, `extrasSchemaVersion: 1` and `maxSeedBytes: 33554432` (32 MiB). The source history remains limited to 10 MiB; the larger seed accommodates the reviewed avatar and extras. New creation/join activation requires extras support before local cleanup. Browser previews and JavaScript exports do not prove native authentication, SQLite or device behavior.
...
**Device-validation finding, 16 September 2026:** real email-code customer accounts use `creationType=null` and a `federated` identity issued by `mail`. The deployed admission correction supports that exact format with a customer-tenant UPN anchor and exact directory revalidation, while retaining legacy local email identities. Confirm the installed phone's end-to-end sign-in against that release. See the [runbook diagnostic and deployment findings](AZURE-MANUAL-SETUP-RUNBOOK.md#follow-up-actual-email-code-account-format-is-rejected-by-current-api). Do not recreate users, switch to password login or bypass identity validation.
...
Public admission uses `Admission__Mode=Directory`; there is **no manual customer allowlist**. Each authenticated request resolves the exact enabled customer object through Graph. Supported formats are (1) `creationType=null`, exactly one `federated`/`mail` identity and one UPN identity issued by the configured customer tenant, or (2) legacy `creationType=LocalAccount` with one `emailAddress` identity issued by that tenant. UPNs are never used as mailbox proof. Other federation, guest creation types, mixed/duplicate login methods and unsupported identities are rejected. The email lookup uses the validated identity issuer (`mail` for OTP), requires one result without pagination, and revalidates object ID, enabled state, format and email. Token `email`/`preferred_username`, Graph `mail` and `otherMails` are not recipient proof. Graph ignores issuer in email-address filters, so returned identities are rechecked. [Graph identity semantics](https://learn.microsoft.com/en-us/graph/api/resources/objectidentity?view=graph-rest-1.0).
...
Shared extras require `0003_FamilySharedExtras.sql`, which expands existing record constraints without replacing family history. No new Azure storage service, secret or registration is required. Confirm the migration in the release journal before deploying the extras-capable API and mobile update.
...
- Create with every record kind, original IDs/decimals, avatar, reminder rules/settings, play selections/check-ins and stopped initial timers. Interrupt the response/restart; retry the same operation and confirm one family/import, complete snapshot activation and no early personal-data cleanup.
- Explicitly accept/decline invitations. Verify five non-admin places are shared by active members and distinct live pending invitations, including concurrent requests and released places. Verify the intended full family installs durably before personal data is cleared, personal history/extras are never uploaded or merged and documented cleanup completes. Cover first sign-in on another device under the existing policy, offline behavior and storage failure; sign-in alone does not add a new destructive migration.
- Verify avatar decoding and shared play state on both phones. Family reminders must remain off until enabled on each phone; test real notification permission, timing, duplication and cleanup after departure. See the [two-phone acceptance procedure](AZURE-MANUAL-SETUP-RUNBOOK.md#c-two-phone-acceptance-after-deployment).
...
Use the runbook's recorded deployment/build evidence to avoid repeating completed setup. Two-device acceptance, real deletion, secret rotation, retention/restore proof and release disclosures remain open until their own evidence exists. Local tests and successful publication do not complete those checks.
