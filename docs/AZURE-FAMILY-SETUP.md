...
**Device-validation finding, 16 September 2026:** real email-code customer accounts use `creationType=null` and a `federated` identity issued by `mail`. The admission correction supports that exact format with a customer-tenant UPN anchor and exact directory revalidation, while retaining legacy local email identities. Deploy the correction before retrying the installed iPhone build; end-to-end device sign-in still requires verification. See the [runbook diagnostic findings](AZURE-MANUAL-SETUP-RUNBOOK.md#follow-up-actual-email-code-account-format-is-rejected-by-current-api). Do not recreate users, switch to password login or bypass identity validation.
