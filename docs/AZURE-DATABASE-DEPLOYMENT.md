...
- The security release added `0004_FamilyAvailabilityBounds.sql`. The next reviewed change is `0005_QueryIndexesAndOperationCounts.sql`, documented in [the index/scaling release guide](DATABASE-SCALING-2026-09-17.md). Deploy its database migration before the matching API. Subsequent migrations start with `0006_DescriptiveChange.sql` under `server/LittleDays.DatabaseMigrator/Scripts`. They are embedded automatically. Never edit, rename, delete or insert before applied migrations. Add a test for the intended schema/data behavior.
- `dbo.DatabaseMigrations` stores ordered script names, normalized SHA-256 checksums and applied timestamps. Repeated runs skip already-applied scripts. CRLF/LF checkout differences are normalized. The migrator also verifies the approved, version-specific indexes, constraints and counter trigger during pending/no-op checks and before journaling each script in the migration transaction. Update `SchemaVerifier` with a new migration that intentionally changes these definitions; an unapproved added, removed or altered index on a managed table blocks release. This is not a complete data audit or a replacement for controlled SQL changes.
