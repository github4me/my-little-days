...
Directory credentials belong to a separate confidential app in the **customer external tenant**. For the initial release, store the secret directly in restricted server-side App Service settings (`AccountDeletion__GraphClientId` / `AccountDeletion__GraphClientSecret`); no Key Vault resource or vault role is required. Keep the value out of source/templates, mobile/EAS, artifacts and logs; restrict configuration/deployment access and rotate it before expiry. Hosting-tenant managed identity remains for SQL; it cannot be assumed to work directly across tenants. Deletion accepts only exact admitted customer object IDs, never administrator identities. Omitting Key Vault does not waive directory deletion or its live release validation.
