...
const db = () =>
  (database ??= (async () => {
    ...
    await d.execAsync(
      "PRAGMA journal_mode = WAL; CREATE TABLE IF NOT EXISTS pilot_accounts (account_id TEXT PRIMARY KEY NOT NULL, payload TEXT NOT NULL);",
    );
    return d;
  })());
...
export async function clearPilot(accountId: string): Promise<void> {
  return serialized(async () => {
    await (
      await db()
    ).runAsync("DELETE FROM pilot_accounts WHERE account_id = ?", accountId);
  });
}
